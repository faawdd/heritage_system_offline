<template>
  <section>
    ...
    <el-dialog v-model="dialog.visible" title="编辑不可移动文物档案" width="820px">
...
async function loadRows() {
  loading.value = true
  try {
    const result = await fetchHeritageSiteManageList(buildParams())
    if (!result.success) {
...
async function handleExport() {
  ...
  try {
    const response = await exportHeritageSiteManage({
      keyword: filters.keyword,
      category: filters.category,
      level: filters.level
    })
    ...
    link.href = url
    link.download = `不可移动文物档案_${new Date().toISOString().slice(0, 10)}.csv`
...
async function beforeImport(file) {
  ...
  try {
    const result = await importHeritageSiteManage(file)
    if (!result.success) {
